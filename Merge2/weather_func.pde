...
void weather() {
  ...
  if (weatherDescription.equals("Fog")){
  ...
  if(weatherDescription.equals("Clear")){
    ...
    for (int i = 0; i < outdoor_array.length; i++) {
        fill(C3);
        rect(50, 470+ i*90, 380, 80, 20);
        fill(C1);
        text(outdoor_array[i].name+"\n"+outdoor_array[i].rating+"\n"+outdoor_array[i].address, 230, 500+i*90);
    }
