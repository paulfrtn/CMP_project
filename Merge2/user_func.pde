//Load an already created user -> sign up
void load_users(JSONArray users) {
  if (users != null) {
    ...
    for (int i = 0; i < users.size(); i++) {
      ...
      JSONArray favorites = userJSON.getJSONArray("favorites");

      users_array[i] = new User(user_id, first_name, last_name, email, pseudo, password, age, address, phone_num, favorites);
    }
  } else {
    ...
    for (int i = 0; i < users_array.length; i++) {
      if (session_pseudo.equals(users_array[i].pseudo)) {
        if (session_password.equals(users_array[i].password)) {
          current_user = new User(users_array[i].user_id, users_array[i].first_name, users_array[i].last_name, users_array[i].email, users_array[i].pseudo, users_array[i].password, users_array[i].age, users_array[i].address, users_array[i].phone_num, users_array[i].favorites);
              if(users_array[i].favorites != null)
              {
                for (int j = 0; j < users_array[i].favorites.size(); j++) {
                  int favoriteID = users_array[i].favorites.getInt(j);
                  favorites_array = append(favorites_array, favoriteID);
                }
                for(int k=0; k<favorites_array.length; k++){
                  println(favorites_array[k]);
                }
              }
